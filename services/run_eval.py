...
- RAG answers generated by local Ollama (no rate limits)
- Judging done by Gemini (needs GOOGLE_API_KEY)

Usage:
    python services/run_eval.py              # reranker ON
    python services/run_eval.py --no-reranker  # reranker OFF
"""
...
def main() -> None:
    ...
    # Config
    # ------------------------------------------------------------------ #
    ollama_model    = os.getenv("OLLAMA_MODEL", "llama3.2:3b")   # RAG uses Ollama
    judge_model     = os.getenv("GEMINI_MODEL_JUDGE", "gemini-2.0-flash")  # judge uses Gemini
    collection_name = os.getenv("RAG_COLLECTION", "pdf_documents")

    # Gemini API key only needed for judge
    api_key = os.getenv("GOOGLE_API_KEY")
    if not api_key:
        print("ERROR: Please set GOOGLE_API_KEY for the judge agent.")
        raise SystemExit(1)
    ...
    # ------------------------------------------------------------------ #
    # Initialize RAG agent (Ollama — no gemini_api_key needed)
    # ------------------------------------------------------------------ #
    rag_agent = RAGAgent(
    ...
    for idx, row in df.iterrows():
        ...
        print(f"Judge Result: {judge_label} -> {is_correct}")
        print("-" * 80)
    ...
    accuracy_row = pd.DataFrame([{
        ...
        "question":         "",
        "gold_answer":      "",
        "predicted_answer": f"{correct_count}/{total}",
...
#
#     # Blank separator row
#     separator = pd.DataFrame([[""]*len(results_df.columns)], columns=results_df.columns)
#
...
# python services/run_eval.py
#
# run_eval.py   Task 1 — answer accuracy
# run_chunking_experiment.py   Task 2 — chunking
# run_retrieval_eval.py    Task 3 — retrieval accuracy
# """