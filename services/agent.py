"""
RAG Agent with Ollama (local) and Qdrant retrieval + cross-encoder reranking.
...
class RAGAgent:
    """
    RAG Agent using local Ollama LLM + Qdrant + Cross-Encoder Reranker.

    Pipeline:
        1. Embed query with Ollama (nomic-embed-text)
        2. Retrieve top-K candidates from Qdrant (vector similarity)
        3. Rerank with cross-encoder (true relevance scoring)
        4. Pass top-N reranked docs as context to local Ollama LLM
    """
    def __init__(
        self,
        collection_name: str = "pdf_documents",
        ...
        # Local Ollama LLM — no API key needed, no rate limits
        self.llm = ChatOllama(
            model=model_name,
            temperature=0.0,
            timeout=300,
    ...
    def answer_question(self, question: str, verbose: bool = False) -> str:
        ...
        if self.use_reranker and self.reranker and raw_docs:
            ...
            if verbose:
                ...
                for i, d in enumerate(docs, start=1):
                    print(f"  {i}. {d['source']} (rerank_score={d['rerank_score']:.3f}, vector_score={d['score']:.3f})")
        else:
            docs = raw_docs
        ...
        prompt = f"""You are a helpful assistant. Answer the question directly and clearly based on the context below.
        ...
        Instructions:
        - Answer the question directly in your own words. Do NOT say things like "Document 1 says..." or "according to source...".
        - Just give a clear, natural answer as if you already know the information.
        - If the context does not contain enough information, say: "I don't have enough information to answer this question."
        - Be concise, 2-4 sentences maximum.

        Answer:"""

        # Stage 4: Generate answer with local Ollama — no retry needed, no rate limits
        try:
